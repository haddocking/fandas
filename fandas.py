...
def main():
    ...
    parser.add_argument(
        "-i", help="input sequence as a text file (REQUIRED)", required=True
    )
    parser.add_argument(
        "-ss",
    ...
    parser.add_argument(
        "-bt", help="BMRB tables as a text file in NMR Star format", default=""
    )
    parser.add_argument(
        "-btc",
    ...
    parser.add_argument(
        "-exp_2dd",
        help="list of distance encoded (distance list, -dl and limit -dlm  must be provided) 2D experiments: %s"
        % list_2dd,
        nargs="+",
        default="",
    )
    ...
    parser.add_argument(
        "-exp_3dd",
        help="list of distance encoded (distance list, -dl and limit -dlm must be provided) 3D experiments: %s"
        % list_3dd,
        nargs="+",
        default="",
    )
    ...
    if exp_2d != []:
        for experiment in exp_2d:
            ...
            if experiment == "NH":
                peaks_proton_heavy(chem_shifts, "H", "N", "H")
            elif experiment == "HN":
                peaks_proton_heavy(chem_shifts, "H", "N", "N")
            elif experiment == "CH":
                peaks_proton_heavy(chem_shifts, "H", "C", "C")
            elif experiment == "HC":
                peaks_proton_heavy(chem_shifts, "H", "C", "H")
            elif experiment == "HH":
                hh(chem_shifts)
            elif experiment == "DQSQ":
                dqsq(chem_shifts)
            elif experiment == "CC_SPINDIFF_INTRA":
                cc_spindiff_intra(chem_shifts)
            elif experiment == "CC_SPINDIFF_INTER":
                cc_spindiff_inter(chem_shifts)
            elif experiment == "NCA":
                nca(chem_shifts)
            elif experiment == "NCO":
                nco(chem_shifts)
    ...
    if exp_3d != []:
        for experiment in exp_3d:
            ...
            if experiment == "NCACX":
                ...
                ncacx_inter(chem_shifts, 3)
            elif experiment == "NCOCX":
                ncocx(chem_shifts, 3)
            elif experiment == "NCOCA_CB":
                ncoca_cb(chem_shifts, 3)
            elif experiment == "SQSQSQ_INTER":
                sqsqsq_inter(chem_shifts)
            elif experiment == "DQSQSQ_INTRA":
                dqsqsq_intra(chem_shifts)
            elif experiment == "DQSQSQ_INTER":
                dqsqsq_inter(chem_shifts)
            elif experiment == "CANH":
                canh(chem_shifts, 3)
            elif experiment == "CONH":
                conh(chem_shifts, 3)
            elif experiment == "CACONH":
                caconh(chem_shifts, 3)
            elif experiment == "COCANH":
                conh(chem_shifts, 3)
            elif experiment == "NCAH":
                ncah(chem_shifts, 3)
    ...
    if (exp_2dd != []) and (dist_list != []):
        for experiment in exp_2dd:
            ...
            if experiment == "CC_SPINDIFF":
                cc_spindiff_dist(chem_shifts, dist_list, distances)
            elif experiment == "HH":
                hh_dist(chem_shifts, dist_list, distances)
            elif experiment == "CHHC":
                chhc_dist(chem_shifts, dist_list, distances)
            elif experiment == "NHHC":
                nhhc_dist(chem_shifts, dist_list, distances)
            elif experiment == "CHH":
                chh_dist(chem_shifts, dist_list, distances)
            elif experiment == "NHH":
                nhh_dist(chem_shifts, dist_list, distances)
            elif experiment == "HHC":
                hhc_dist(chem_shifts, dist_list, distances)
            elif experiment == "NCACX":
                ncacx_dist(chem_shifts, 2, dist_list, distances)
            elif experiment == "NCOCX":
                ncocx_dist(chem_shifts, 2, dist_list, distances)
            else:
                print("%s is not a valid experiment" % experiment)
    ...
    if (exp_3dd != []) and (dist_list != []):
        for experiment in exp_3dd:
            if experiment == "CHH":
                chh_dist_3d(chem_shifts, dist_list, distances)
            elif experiment == "NHH":
                nhh_dist_3d(chem_shifts, dist_list, distances)
            elif experiment == "NCACX":
                ncacx_dist(chem_shifts, 3, dist_list, distances)
            elif experiment == "NCOCX":
                ncocx_dist(chem_shifts, 3, dist_list, distances)
            else:
                print("%s is not a valid experiment" % experiment)
                        ...
                        for (
                            ...
                            if i != 0:
                                if (
                                    ...
                                    and chem_shifts[i][c_ind] != 0
                                    and chem_shifts[i][j] != 0
                                ):  # check if the chemical shifts of the three are not zero
                                    shift_1.append(
                                        [residue, i + 1, chem_shifts[i][j], atoms[j]]
                                    )
                                    shift_2.append(
